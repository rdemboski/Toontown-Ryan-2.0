...
def getValidLevelsByCog():
    validLevels = []
    i = 1
    for level in MeritsPerLevel:
        if len(level) < 6:
            validLevels.append(range(i, i + 5))
            i += 1
        else:
            validLevels.append(range(8, 51))
            i = 1
    validLevelsByCog = dict(zip(SuitDNA.suitHeadTypes, validLevels))
    return validLevelsByCog