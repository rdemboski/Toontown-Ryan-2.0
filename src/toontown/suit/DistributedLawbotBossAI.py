...
class DistributedLawbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def hitBoss(self, bossDamage):
        ...
        if self.bossDamage >= self.bossMaxDamage:
            ...
            logger.log("Scale = %s" % (self.bossDamage), whisper=1, suites=["debugs-on"])
        else:
            self.__recordHit()
            logger.log("Scale = %s" % (self.bossDamage), whisper=1, suites=["debugs-on"])
...
def restartScale():
    ...
    if not boss:
        return "You aren't in a CJ!"
    boss.exitIntroduction()
    boss.b_setState('RollToBattleThree')
    return 'Restarting the scale round...'
