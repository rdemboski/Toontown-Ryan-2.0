...
class DistributedBossbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def hitBoss(self, bossDamage):
        ...
        if self.bossDamage >= self.bossMaxDamage:
            ...
            logger.log("HP = %s" % (self.bossMaxDamage - self.bossDamage), whisper=1, suites=["debugs-on"])
        else:
...
def restartSeltzer():
    ...
    for table in boss.tables:
        table.turnOn()
        table.goFree()
    boss.b_setBossDamage(0, 0, 0)
    boss.doneOvertimeOneAttack = False
    boss.doneOvertimeTwoAttack = False
    boss.b_setState('BattleThree')
    boss.b_setState('PrepareBattleFour')
